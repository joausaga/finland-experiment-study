#######
# An analysis of participants' level of group efficacy
#######
...
# Data preparation
survey_2 = raw_survey_2 %>% select(email,Q4_7)
survey_3 = raw_survey_3 %>% select(email,Q7_7)
melt_s2 = melt(survey_2, id.vars='email', measure.vars=c('Q4_7'))
melt_s3 = melt(survey_3, id.vars='email', measure.vars=c('Q7_7'))
melt_s2$survey = 'survey_2'
melt_s3$survey = 'survey_3'
dataset = rbind(melt_s2,melt_s3)
dataset$variable = 'Different viewpoints related\nto the law were brought up in the conversation'
...
# Draw a boxplot
ggplot(dataset, aes(x=variable, y=value, fill=variable)) + 
  geom_boxplot() + 
  facet_grid(. ~ survey) +
...
# Analyze correlations between group efficacy (survey 2) and changes in the motivation factors
...
find_correlations(ch_mot_factors,survey_2_,ch_factors,c('Q4_7'))

# Analyze correlations between group efficacy (survey 3) and changes in the motivation factors
survey_3_ = survey_3 %>% filter(email %in% ch_mot_factors$email)
find_correlations(ch_mot_factors,survey_3_,ch_factors,c('Q7_7'))
